...
- The project's own files are named relative to the working directory (`BLUEPRINT.md`, `src/main.py`). read_file, write_file and edit_file take any absolute path too; a file only root may write is written for you. Where projects live is changed with `set_setting`, only when the user asks for it.
...
pub fn job_turn(instructions: &[String], job: &Job, blueprint: Option<&str>, last_run: Option<&str>) -> Prompt {
    ...
    let (header, bp_block) = if job.housekeeping {
        ...
        (format!("Project: {}. Its folder {} is already made and is the working directory: every file of the project goes in it, named relative to it (BLUEPRINT.md, src/main.py). Never make another folder for it, never write it anywhere else, and never change projects_root.", job.project, job.folder), format!("\n\nBLUEPRINT.md:\n{bp}"))
    };
