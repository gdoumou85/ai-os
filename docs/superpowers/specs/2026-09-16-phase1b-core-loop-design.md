...
**A failure is told in full, once, and never retried as-is.** A step that fails once will fail again, so the first failure hands the model the whole reason — for commands, the exit code and the *end* of the error output, where the reason lives (the executor's 500-character cut is taken from the tail for failures, not the head). The model must then do something **different**; the loop refuses an action identical to one that already failed **unless something has changed since** — a file written or edited successfully after the failure makes a retry legitimate (spike finding 2026-09-16: after fixing `prnt`→`print`, re-running the script *is* the right move). So the refusal list is cleared by every successful file change. **Giving up is a fixed count, not the model's mood:** 3 *different* failed attempts at the same plan step, or 25 steps in total, and the job stops as *failed* with a plain-language reason.
...
## 8b. Spike result (2026-09-16, `trial/loop-dryrun.py`, throwaway)
qwen3.5:9b drove this exact loop — the plan's system text, prompts and schema, commands through the real 1a sandbox — for the primes job. Three runs: creative / creative with a planted `prnt` bug / ask-mode with thinking on for planning. **Two of three finished proven done in 8–10 turns (≈2–4 s per move, ~1k tokens in);** the third failed only because the identical-retry rule refused a legitimate re-run after a fix (rule corrected in §3). Observed: sensible front-door `start`; a 3–4 step plan; a correct script first time; blueprint created first and updated **in place with `edit_file`** when the loop refused `done`; the planted bug fixed with a one-line `edit_file` straight from the traceback; a real check (`python3 primes.py`), never a lazy one. Weak spots: every run first tried `python` (not on Ubuntu) — the job prompt now states the machine; one replan was written as prose rather than steps; thinking on for planning cost 2.4× on that turn for no visible gain — kept off.
