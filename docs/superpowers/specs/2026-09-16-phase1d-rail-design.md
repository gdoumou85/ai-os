...
7. **One retry for the blueprint gate (runs 7 and 8, the same failure twice).** With the write done, the model said `done`, the gate held it back for the BLUEPRINT update, it said `done` again, and the grammar budget of two ended the job. A `done` held back by the gate is a legal move refused by policy, not an answer the system could not read, and what it asks for is one concrete extra step — so it has its own bound now (`Job::done_gated`, `MAX_DONE_GATED = 4`), cleared like `rejections` whenever an action really runs, so it bounds a model getting nowhere rather than counting a long job's honest gate hits. The note tells the model to do it with an act and then say done again. A model that only ever says done still ends.
...
**Run 10, 68.1 s, passed too** — the same four scripts after the review's three findings were fixed: `make_dir` refused for any relative path rather than only one pointing inward; `cp`/`mv` judged on their destination so a copy of the machine into the project stays free; `done_gated` cleared on progress. Two passing runs out of two on the finished code.
...
The acceptance hangs rather than fails when the engine waits where the script expects it to finish: `say_until` blocks in `next_event()` and its own deadline is only checked when an event arrives, so a `needs_ok` the script did not plan for leaves it with nothing to wake it. Every run here was wrapped in `timeout` for that reason. A read deadline on the client socket turns it into an ordinary failure: added after the merge (2026-09-17) as `Client::set_read_timeout`, armed per call by `say_until`; the rail never sets one.
