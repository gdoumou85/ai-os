...
pub fn front_door(instructions: &[String], projects: &[ProjectRow], notebooks: &[String], recent: &[(String, String)], message: &str) -> Prompt {
    ...
    let user = format!(
        "Standing instructions:\n{}\n\nProjects the user named:\n{}\n\nSkill notebooks: {}\n\nRecent exchange:\n{}\n\nLegal moves now: reply (just talk: it runs nothing, so when the user wants something done — 'do it', 'proceed' — start or housekeep instead), start (a project: something to build and keep as files — code, documents, a site, a game — new or more work on one; a new project, or a folder for one, is always start, never housekeep, and its folder is made for you: give project, new_project, description, goal, creative, understood, and skills (0-3 craft areas the job belongs to — coding, web design, a program like blender — named from the notebooks listed or a new short name; [] for a plain errand)), or \
         housekeep (the machine itself, never making a project: its files and folders, listing, moving or deleting projects already there, settings, tools, or a program on the desktop — a window the user named, or one you open yourself to do what was asked, a browser and the websites in it included; give goal, understood). \
         ...
         Pick a project listed there when the user means it; when the user says where a project is or is to be, give the absolute path of the project's own folder as folder. Set creative=true only if the user said to decide yourself. \
