...
**The live acceptance has passed** (1c design §11 Results): qwen3.5:9b, both hands, no human, four scripts in 140s — it prepared `/data/work` and moved new projects there, wrote and ran a primes script inside it, installed cowsay through the wrapper and proved it, gave all of it back on the word *undo* one job at a time, and pulled `tabulate` from PyPI through the registry allowlist to print a table. Fifteen runs went into getting there: the nine gaps the failures exposed (the model reaching for `mkdir`, `apt` and absolute paths instead of the hands; `/usr/games` off the sandbox PATH; a successful `make_dir` whose detail was empty; the jail's hidden `/data` read as the machine's truth; a workspace-relative program never running; the front door summarising a request's standing half away) are all fixed in code, not in the test, and are numbered in that Results block — along with the one wording question still open there.
