...
| U8 | GNOME or KDE | DECISION — **GNOME** for the WSL edition; revisit KDE on bare metal | GNOME proven end-to-end here (headless shell renders apps, AT-SPI full read + action, Mutter capture → real PNG). KDE's compositor, a11y bus and `org.kde.KWin.ScreenShot2` (a *cleaner*, dialog-free capture than GNOME's) all come up headless, but a bare `kwin_wayland --virtual` is not a full session, so GTK apps can't open a display and never register — plumbing, not an accessibility limit. See Task 6 + Task 8 |
...
## Task 6: KDE probes (U8 input)
`trial/headless-kde.sh` stands up an invisible `kwin_wayland --virtual 1600x900` session under the user systemd manager and starts the AT-SPI stack by hand (`at-spi-bus-launcher` + `at-spi2-registryd`), since there is no Plasma session to do it.
- **Compositor:** kwin_wayland comes up headless with no GPU, accepting clients on `wayland-1`. Fast, light.
- **Bus surface (the tier-3 fallback question):** `org.kde.KWin.ScreenShot2` is exposed **directly by kwin_wayland**, to the session owner, **with no dialog** — this is cleaner than GNOME 50, where `org.gnome.Shell.Screenshot` is blocked and capture must go through the heavier Mutter `ScreenCast`+PipeWire route. The `kde.portal` backend and `org.freedesktop.portal.Desktop` are present/activatable.
- **App rendering under bare kwin: FAILED (plumbing, not accessibility).** LibreOffice (with `libreoffice-gtk3` installed, `SAL_USE_VCLPLUGIN=gtk3`, `GDK_BACKEND=wayland`, `DISPLAY` unset) logs "Failed to open display" and never opens a window, so it never registers on the a11y bus (`pyatspi` desktop shows only `xdg-desktop-portal-gtk`). A bare `kwin_wayland --virtual` is only a compositor — no Plasma shell, no full portal/session wiring — so GDK can't complete a Wayland connection. Bringing up a full headless Plasma (`startplasma-wayland`) is the same class of session-plumbing effort already ruled out on the GNOME RDP side (attempt A/B), and it is **not needed to answer U3/U4**: AT-SPI is toolkit-bound (U3), so the same probe that read LibreOffice's 2099 nodes and toggled Bold under GNOME reads it identically under KDE the moment the app has a window. The failure here is "no window," not "no tree."

## Task 8: GNOME vs KDE decision
**Decision: GNOME for the WSL development edition. Re-evaluate KDE for bare metal.**

Why GNOME now:
- It is **proven end-to-end in this exact environment**: `gnome-shell --headless --virtual-monitor` renders real apps, LibreOffice registers a full AT-SPI tree, the AI reads it and drives it via actions, and `org.gnome.Mutter.ScreenCast` produced a valid 277 KB PNG of the real screen (U7 capture). Nothing in the primary control path is unproven.
- The one thing that decides desktop choice is the **tier-3 screen fallback** (U5), because the primary path (AT-SPI, U3/U4) is identical on both. GNOME's fallback (Mutter ScreenCast/RemoteDesktop) is proven working; that is enough to ship the dev edition.

Why KDE is the bare-metal candidate to revisit (not chosen now):
- `org.kde.KWin.ScreenShot2` is a **dialog-free, session-owner capture exposed by the compositor itself** — simpler than GNOME's PipeWire route, and GNOME's own `Shell.Screenshot` is blocked. On bare metal (real DRM backend + real seat) KDE's rendering-plumbing issue disappears, and its lighter compositor + cleaner capture interface may win.
- This is a **low-stakes, reversible** call: because AT-SPI is desktop-independent, switching desktops later changes only the tier-3 fallback binding, not the AI's core control code.

State left behind: the invisible `ai-headless-kwin` unit is still up from Task 6; the primary workshop session is GNOME (`ai-headless-shell`, `trial/headless-session.sh`). Restart the GNOME session before further Phase-0/Phase-1 work.
